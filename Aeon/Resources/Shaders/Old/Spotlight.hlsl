...
float3 main(VertexOutput input) : SV_TARGET
{
    ...
    const float3 lightContribution = CalculateLight(CB_CameraPos, worldPos, normal, l, diffuseColor, specularColor, SLB_Color, PLB_Intensity, roughness);
    
    const float coneAttenuation = pow(saturate(saturate(dot(SLB_Direction, l) - cos(SLB_ConeAngle)) / max(SLB_ConeAngleDiff, 0.0001f)), 2.0f);
    const float rangeAttenuation = saturate(1.0f - pow(d * (1.0f / max(SLB_Range, 0.001f)), 2.0f));
    const float normalAttenuation = saturate(dot(-l, normal));
    
    return float4(lightContribution * coneAttenuation * rangeAttenuation * normalAttenuation, 1.0f);
}
